        ...
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                let internalName = this.getPreferredStringName(names.postScriptName) ||
                    ...
                    font.designation ||
                    font.idFont;
                console.log(`[NodeAdapter] Font internal name (from font file): "${internalName}"`);
                ...
                // Step 5: Register with node-canvas using INTERNAL NAME
                // CRITICAL: Use internalName (from font file) because fabric JSON has fontFamily: "Book Antiqua"
                // NOT the GUID! This ensures fabric.js can find the font when rendering.
                // registerFont(tempPath, { family: internalName });
                // console.log(`[NodeAdapter] Font registered with family name: "${internalName}" (GUID: ${font.idFont})`);
                // Step 6: Verify font is registered and available using INTERNAL NAME
                yield this.verifyFontRegistration(internalName, tempPath);
                // Cache the path for reuse (by GUID for lookup)
                this.fontPathCache.set(font.idFont, tempPath);
                console.log(`[NodeAdapter] Font ${font.idFont} successfully registered and verified as "${internalName}"`);
            }
            catch (error) {
...
//# sourceMappingURL=canvas-renderer-node.adapter(1).js.map