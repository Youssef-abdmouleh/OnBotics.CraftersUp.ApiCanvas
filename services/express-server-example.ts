...
import { FontManager, globalFontManager, FontDefinition } from './font-manager';
 ...
 */
const preloadFontsMiddleware = async (req: Request, res: Response, next: Function) => {
  try {
    ...
    const fontPromises = fonts.map(async (font: FontDefinition) => {
      try {
 ...
 * }
 */
app.post('/api/render', preloadFontsMiddleware, async (req: Request, res: Response) => {
  let adapter: CanvasRendererNodeAdapter | null = null;
  ...
  try {
    ...
    // Créer l'adaptateur
    adapter = new CanvasRendererNodeAdapter();
    ...
    console.log(`[API] Buffer: ${result.buffer?.length} octets`);
    console.log('[API] ========================================');
 ...
 * POST /api/test-font
 */
app.post('/api/test-font', async (req: Request, res: Response) => {
  try {
    const { font }: { font: FontDefinition } = req.body;
    ...
    const fontInfo = globalFontManager.getFontInfo(font.idFont);
    ...
    res.json({
      ...
      font: {
        ...
        internalName: fontInfo?.internalName,
        isRegistered: fontInfo?.isRegistered,
...
app.get('/api/cache-stats', (req: Request, res: Response) => {
  ...
  res.json({
    ...stats,
    memorySizeMB: (stats.memorySize / (1024 * 1024)).toFixed(2),
    diskSizeMB: (stats.diskSize / (1024 * 1024)).toFixed(2)
  });
});
